...
use function vnode\{el, multival, comma};
...
describe("`el(1234567)` or `el([])`", function () {
    it("throws a `TypeError`, the first argument has to be a string", function() {
        expect(function () { el(1234567); })->toThrow(new TypeError());
        expect(function () { el([]); })->toThrow(new TypeError());
    });
});
...
describe("el('a', ['href'=>'/link', 'id'=>'Anchor', 'class'=>'btn primary'])", function () {
    ...
    describe('the attribute key', function () {
        it("is always a string", function() {
            foreach ($this->actual->attributes as $key => $val) expect($key)->toBeA('string');
        });
    });
});
...
describe("el('h1', 'text child', el('br'))", function () {
    describe("multiple parameters, where the first one is not `vnode\AttributeMultiValue` or `array`\nare considered children", function () {
